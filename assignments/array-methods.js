...
runners.forEach((curVal) => {
  return fullNames.push(`${curVal.first_name} ${curVal.last_name}`);
});
console.log(fullNames);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase because the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. This array will contain just strings.
let firstNamesAllCaps = runners.map(function(curVal) {
...
let runnersLargeSizeShirt = runners.filter(function(curVal){
...
let ticketPriceTotal = runners.reduce(function(accumulator, curVal){
  return accumulator + curVal.donation;
}, 0);
...
// Problem 1
//Generate list of emails for mailing list
let emailList = [];
runners.forEach(function(curVal){
  emailList.push(curVal.email);
});
console.log(emailList);
// Problem 2
// Filter out large donors to give a special award to
const largeDonation = runners.filter(function(curVal){
  return curVal.donation > 200;
});
console.log(largeDonation);

// Problem 3
//See how many people from your company are racing
const yourCompany = runners.filter(function(curVal){
  return curVal.company_name === "Kanoodle";
});
